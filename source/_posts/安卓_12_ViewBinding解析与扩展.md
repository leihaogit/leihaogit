...
`视图绑定（ViewBinding）`是一种在安卓开发中用于访问布局文件中视图的机制，使用非常简单，只需要几行代码就可以了。如果只是单纯介绍 ViewBinding 的使用，是完全没必要写一篇博客专门来介绍的，所以这篇文章会更深入的剖析一下 ViewBinding 实现原理，强调一些使用中需要注意的地方，以及自己实现一个`by viewModel()`类似的视图绑定扩展`by viewBinding()`。
